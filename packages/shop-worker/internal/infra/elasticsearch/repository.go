...
func (r *esRepository) SyncNestedData(ctx context.Context, param port.NestedParam) error {
	painlessScript := `
	...
	_, err := r.client.Update(param.Index, param.DocID).
		Script(&types.Script{
			...
			Params: map[string]json.RawMessage{
				...
				"data":    json.RawMessage(r.MustMarshal(param.Data)),
			},
		}).
		Upsert(upsertData).Do(ctx)
...
func (r *esRepository) MustMarshal(value any) json.RawMessage {
