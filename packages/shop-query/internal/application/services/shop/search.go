...
type SearchService struct{ esService esx.ESXService }

func NewSearchService(esService esx.ESXService) *SearchService {
	return &SearchService{esService: esService}
}
func Term(field string, value any) Filter { return Filter{"term": map[string]any{field: value}} }
...
func SearchDocuments[T any](ctx context.Context, service *SearchService, spec SearchSpec) (*PageResult[T], error) {
	...
	return &PageResult[T]{Items: items, Total: total, NextPageToken: nextPageToken(page, len(items), total)}, nil
}
...
func buildSearchBody(page port.Page, filters []Filter, must []Filter, sorts []Sort) map[string]any {
	...
	body := map[string]any{"from": pageOffset(page.Token), "size": page.Size, "track_total_hits": true, "query": map[string]any{"bool": boolQuery}}
	if len(sorts) > 0 {
		body["sort"] = buildSorts(sorts)
	}
	return body
}
func buildSorts(sorts []Sort) []map[string]any {
	...
	for _, sort := range sorts {
		...
		result = append(result, map[string]any{sort.Field: map[string]any{"order": string(direction)}})
	}
	return result
}
